...
def calc_triplets_and_hist(sample, rew_samples, pt_jet_ranges, max_delta_zeta=float('nan'), delta_legs=float('nan'),
    ...
    hists_event_weight = [[ROOT.TH1D("Event weights ({:}, {:})".format(i, j), "event-weight", nbins, -1, 1) for j in range(len(rew_samples))] for i in range(2)]
            ...
            if hadronic_jet_idx is not None:
                ...
                if len(jet_constituents) > 0:
                    ...
                    for h, mtop_bw in enumerate(rew_samples):
                        ...
                        hists_jet_pt[g].Fill(hadronic_jet_pt, event_weight*bw_weight)
                        hists_jet_mass[g].Fill(hadronic_jet_mass, event_weight*bw_weight)
                        hists_event_weight[g][h].Fill(event_weight*bw_weight)
                        count += 1
...
def save_root_hists(hists_top, hists_w, hists_jet_pt, hists_jet_mass, hists_varied, pt_variations, hists_ev_weight,
                    rew_values, pt_jet_ranges, filename):
    ...
    for g, level in enumerate(['Gen', 'PF']):
        ...
        for h, mtop_bw in enumerate(rew_values):
            hists_ev_weight[g][h].Write('event_weights_{:}_{:}'.format(level, mtop_bw))
...
if __name__ == '__main__':
    ...
    save_root_hists(hists_top=hists, hists_w=hists_w, hists_jet_pt=hists_jet_pt, hists_jet_mass=hists_jet_mass,
                    ...
                    rew_values=rew_samples, pt_jet_ranges=pt_jet_ranges,
                    filename='histogram_files/correlator_hist_trip_21_pp_{:03}_{:}.root'.format(args.job, args.sample_type))
    end = time.time()
